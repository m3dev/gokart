...
class TestPandasAssert(unittest.TestCase):
    ...
    def test_assert_frame_equal_with_small_error(self):
        ...
        gokart.testing.assert_frame_equal(resulted, expected, atol=1e-1)

    def test_assert_frame_equal_with_duplicated_columns(self):
        expected = pd.DataFrame(data=dict(f1=[1, 2, 3], f3=[111, 222, 333], f2=[4, 5, 6]), index=[0, 1, 2])
    ...
    def test_assert_frame_equal_with_duplicated_indexes(self):
        expected = pd.DataFrame(data=dict(f1=[1, 2, 3], f3=[111, 222, 333], f2=[4, 5, 6]), index=[0, 1, 2])
