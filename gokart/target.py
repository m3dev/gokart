...
class TargetOnKart(luigi.Target):
    ...
    def dump(self, obj, lock_at_dump: bool = True, task_params: Optional[dict[str, str]] = None, user_provided_labels: Optional[dict[Any, Any]] = None) -> None:
        if lock_at_dump:
            wrap_dump_with_lock(func=self._dump, task_lock_params=self._get_task_lock_params(), exist_check=self.exists)(
                obj=obj, task_params=task_params, user_provided_labels=user_provided_labels
            )
        else:
            self._dump(obj=obj, task_params=task_params, user_provided_labels=user_provided_labels)
    ...
    @abstractmethod
    def _dump(self, obj, task_params: Optional[dict[str, str]] = None, user_provided_labels: Optional[dict[Any, str]] = None) -> None:
        pass
...
class SingleFileTarget(TargetOnKart):
    ...
    def _dump(self, obj, task_params: Optional[dict[str, str]] = None, user_provided_labels: Optional[dict[Any, Any]] = None) -> None:
        with self._target.open('w') as f:
            self._processor.dump(obj, f)
        if self.path().startswith('gs://'):
            GCSObjectMetadataClient.add_task_state_labels(path=self.path(), task_params=task_params, user_provided_labels=user_provided_labels)
